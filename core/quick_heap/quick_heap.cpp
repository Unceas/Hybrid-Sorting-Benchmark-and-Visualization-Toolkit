        ...
        while (low < high) {
            ...
            if (size < config.insertionThreshold) {
                if (config.enableSplitsLog) {
                    telemetry.logs.push_back("[INFO] Depth " + std::to_string(depth) + ": Subproblem size " + std::to_string(size) + " < " + std::to_string(config.insertionThreshold) + " -> HeapSort base case.");
                }
...
void quickHeapSort(std::vector<int>& arr, const SortConfig& config, SortTelemetry& telemetry) {
    ...
    if (config.enableSplitsLog) {
        telemetry.logs.push_back("[SYSTEM] Quick+Heap pipeline started. Size N = " + std::to_string(n) + ", Base Threshold = " + std::to_string(config.insertionThreshold) + ".");
    }
    quickHeapUtil(arr, 0, n - 1, config, telemetry, 0);
    if (config.enableSplitsLog) {
        telemetry.logs.push_back("[SYSTEM] Quick+Heap sorted " + std::to_string(n) + " elements. Comparisons: " + std::to_string(telemetry.comparisons) + ", Swaps: " + std::to_string(telemetry.swaps) + ", Max Stack Depth: " + std::to_string(telemetry.maxDepth) + ".");
    }
}
